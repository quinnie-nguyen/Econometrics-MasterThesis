...
class HestonCalibrator(BasicCalibrator, ConditionalPosteriorHestonMixin):
    ...
    def __init__(self, parameter_dict: dict = None, alpha_prior: float = 2.0, beta_prior: float = 0.01, p_prior: float = 2.0,
        ...
        self.vol_prior_mean = vol_prior_mean
        self.vol_prior_std = vol_prior_std
        ...
        if parameter_dict == None:
            ...
            self._params_dict = parameter_dict

    def calibrate(self, n_mcmc_steps=10000, burn_in=5000, fixed_param = None):
        ...
        if fixed_param is None:
            ...
            V_t_array = np.array(truncnorm.rvs(a=0, b=np.inf, loc=self.vol_prior_mean, scale=self.vol_prior_std, size=self.T + 2)) # loc = 0.0225, scale = 0.005
            ...
            for iter in tqdm(range(1, n_mcmc_steps)):
                ...
                for t, (Y_t, Y_t_plus_1, V_t_minus_1, V_t, V_t_plus_1) in enumerate(Y_and_V_arrays):
                    ...
                    V_proposal = np.random.normal(V_t, self.rejection_rate)
            ...
            self._params_dict = fixed_param

    def get_paths_old(self, s0=100, nsteps=2000, nsim=100, risk_neutral=False):
    ...
    def get_paths(self, s0=100, nsteps=2000, nsim=100, risk_neutral=False, v0 = None, dt = 1/252):
        assert self._params_dict is not None, "Parameters have not been calibrated yet"
        mu = self._params_dict.get("mu_final")
        kappa = self._params_dict.get("kappa_final")
        theta = self._params_dict.get("theta_final")
